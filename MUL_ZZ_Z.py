...
def MUL_ZZ_Z(a: ceil, b: ceil) -> ceil:
    ...
    # Получаем абсолютные значения чисел (учет знака ответа будет произведен позже).
    abs_a = ABS_Z_Z(a)
    abs_b = ABS_Z_Z(b)
    
    # Преобразовываем целые числа в натуральные (учет знака ответа будет произведен позже).
    nat_a = TRANS_Z_N(abs_a)
    nat_b = TRANS_Z_N(abs_b)
    
    # Умножаем абсолютные натуральные числа.
