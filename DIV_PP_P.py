...
def DIV_PP_P(a: P, b: P) -> P: # функцию писал в момент, когда еще не были реализованы классы Q и P. потом подстроюсь под них.
    """
    ...
    if DEG_P_N(b) == 0:  # проверяем степень второго полинома, вдруг это просто число. 
        ...
        divider = DIV_QQ_Q(Q(ceil([1], 1, 0), nat_0([1], 1)), x2.coefficients[0])
        return MUL_PQ_P(x, divider)
    
    coef = Q(ceil([0], 1, 0), nat_0([1], 1)) # коэффициент частного при делении полиномов
    diff = 0 # инициализация разницы степеней полиномов
    mid = P(0, [Q(ceil([0], 1, 0), nat_0([1], 1))])
